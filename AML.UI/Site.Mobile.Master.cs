    ...
    public partial class Site_Mobile : MasterPage
    {
        ...
        public string selectedLanguage
        {
            get
            {
                ...
                if (Session["lang"] != null && Session["lang"].ToString().ToLower().Contains("ar"))
                    return AML.Domain.Helper.Languages.Lang.Arabic.ToString();

                return AML.Domain.Helper.Languages.Lang.English.ToString();
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    litLastName.Visible = litLastName2.Visible = spanIcon.Visible = topMenu.Visible = liLogout.Visible = li1.Visible = liManage.Visible = liFAQs.Visible = false;
                }
            }
        }
        ...
        {
            var subCategories = CategoryService.GetById(id, selectedLanguage).SubCategories;
            string outcome = string.Empty;
            foreach (var item in subCategories)
                outcome += " <li><a href=\"" + (item.ContentType.Id == 1 ? Page.ResolveUrl("~/SubCategoryPageContent/Index?Id=" + item.Id) : Page.ResolveUrl("~/SubCategoryPageContent/Media?Id=" + item.Id)) + "\">" + item.GetType().GetProperty(selectedLanguage == AML.Domain.Helper.Languages.Lang.Arabic.ToString() ? "NameArabic" : "NameEnglish").GetValue(item, null) + "</a></li>";
            return outcome;
        }
